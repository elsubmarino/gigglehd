...
<fmt:parseDate value="${dto.dateTime }" var="dateTime" pattern="yyyy-MM-dd'T'HH:mm:ss" type="both"/>
<div class="row">${dto.title } <fmt:formatDate value="${dateTime}" pattern="yyyy-MM-dd HH:mm"/></div>
<div class="row">${dto.writer }${dto.hit }</div>
<div class="row">${dto.content }</div>
<div class="row center-block">
<a href="#" id="updateDelete"><span id="updateDeleteCount">${updateDeleteCount }</span>삭제 요청</a>
<a href="#" id="updateRecommendation"><span id="updateRecommendationCount">${recommendation }</span>추천</a>
</div>
<div class="row">
	<a href="#" onclick="goback()">목록</a> <a href="#">스크랩</a> 
	<c:if test="${param.mainCategory eq 'community' }">
	<a href="/board/write${pageMaker.makeQuery(pageMaker.cri.page) }&sequence=${dto.sequence }&lvl=${dto.lvl }&groupNum=${dto.groupNum}&mode=reply">답글</a>
	</c:if>
	 <a href="#" id="modify">수정</a> <a
		href="#" id="del">지우기</a>
</div>
<div class="row">
	...
	<c:forEach items="${list }" var="item">
		<form method="POST" action="/board/writeComments">
			...
			<input type="hidden" name="lvl" value="${item.lvl + 1 }"> <input
				type="hidden" name="rootNum" value="${item.rootNum }" />
...
<script>
	...
	$(function() {
			...
			$.ajax({
				...
				success:function(res){
					if(res.status=="success"){
						updateDeleteCount.text(Number(updateDeleteCount.text())+1);
					}
				},
		...
		$("#updateRecommendation").click(function(){
			var data={"num":"${dto.num}"};
			...
			$.ajax({
				...
				success:function(res){
					if(res.status=="success")
						updateRecommendationCount.text(Number(updateRecommendationCount.text())+1);
				},
				error:function(xhr,stat,err){
